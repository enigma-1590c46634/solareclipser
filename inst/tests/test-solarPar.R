...
test_that("covlist pvalue", {
  ...
  # a single value is expected, as one covariate is specified in `mod1`
  expect_equal(length(val1), 1)
  expect_true(is.numeric(val2))

  # two values are expected, as two covariates are specified in `mod2`
  expect_equal(length(val2), 2)
})
...
test_that("function explainedVarProp", {
  ...
  # no results for a model without any covariate
  expect_true(all(is.na(tab0$explainedVarProp)))

  # no matter whether covtest is TRUE/FALSE, the results must be for the case
  # of all covariates
  expect_true(any(is.na(tab1$explainedVarProp)))
  expect_equal(sum(!is.na(tab1$explainedVarProp)), 1)

  # if covtest is TRUE, all results must be obtained
  expect_true(all(!is.na(tab2$explainedVarProp)))
})
